...
# ------------------ Helper Function ------------------
def log_login_attempt(username, ip, device, browser, mfa_enabled, outcome, risk_score=None, risk_decision=None, reasons=None, timestamp=None):
...
def main():
    ...
    page = st.sidebar.selectbox("Choose Page", ["Register", "Login", "Train Model", "Admin"])
    ...
    if page == "Register":
        ...
        if st.button("Register") and not st.session_state.registration_done:
            if username.strip() == "" or password.strip() == "":
                ...
                if uid is None:
                    st.error("❌ Registration failed. User already exists or error occurred.")
                else:
    ...
    # ---------------- LOGIN ----------------
        ...
        if st.button("Login", key="initial_login"):
            ...
            if username not in users["Username"].values:
                ...
                log_login_attempt(username, get_ip(), get_device_info(), get_browser_info(), 0, outcome=1, risk_score=1.0, risk_decision="BLOCK", reasons=["User not registered"])
            else:
                user_row = users.loc[users["Username"] == username].iloc[0]

                if not check_password(password, user_row["Password"]):
                    st.error("❌ Incorrect password!")
                    log_login_attempt(username, get_ip(), get_device_info(), get_browser_info(), int(user_row["MFA Enabled"]), outcome=1, risk_score=1.0, risk_decision="BLOCK", reasons=["Incorrect password"])
                else:
                    ...
                    if "ALLOW with MFA" in decision:
                        ...
                        st.session_state.login_info = {
                            "username": username, "ip": ip, "device": device, "browser": browser,
                            "mfa_enabled": mfa_enabled, "risk_score": risk_score, 
                            "risk_decision": decision, "reasons": reasons, "timestamp": timestamp
                        }

                        # Log temporary entry (Outcome=None)
                        log_login_attempt(username, ip, device, browser, mfa_enabled, outcome=None,
                                          risk_score=risk_score, risk_decision=decision, reasons=reasons,
                                          timestamp=timestamp)

                        st.warning("⚠️ Medium risk detected — MFA verification required.")
                        st.session_state.show_mfa_camera = True
                    
                    elif "ALLOW" in decision:
                        # Direct successful login
                        log_login_attempt(username, ip, device, browser, mfa_enabled, outcome=0, risk_score=risk_score, risk_decision=decision, reasons=reasons)
                        st.success(f"✅ Login allowed. Welcome, {username}!")

                    elif "BLOCK" in decision:
                        # Direct block
                        log_login_attempt(username, ip, device, browser, mfa_enabled, outcome=1, risk_score=risk_score, risk_decision=decision, reasons=reasons)
        ...
        if st.session_state.show_mfa_camera and not st.session_state.mfa_verified:
            ...
            st.info("Please capture a photo of your face using the camera below to proceed with MFA.")
            ...
            if mfa_photo is not None:
                ...
                outcome = 0 if success else 1 # 0 is success (allowed), 1 is failure (blocked)

                # Update the temporary CSV entry logged during the initial login
                logins = load_logins()
                
                # Find the unique pending login attempt entry
                idx = logins[(logins["Username"] == info["username"]) &
                             (logins["Risk Decision"] == "ALLOW with MFA") &
                             (logins["Timestamp"] == info["timestamp"])].index
                ...
                if not idx.empty:
                    ...
                    log_login_attempt(info["username"], info["ip"], info["device"], info["browser"], info["mfa_enabled"], outcome=outcome, risk_score=info["risk_score"], risk_decision=updated_decision, reasons=info["reasons"], timestamp=info["timestamp"])

                if success:
                    st.success(f"✅ MFA verification completed — login allowed. Welcome, {info['username']}!")
                else:
                    st.error("❌ MFA verification failed (No face detected). Access denied.")
    ...
    #         st.info("No pending MFA-required logins found.")
        ...
        if logins.empty:
            ...
            blocked_recent = logins[logins["Outcome"] == 1].sort_values("Timestamp", ascending=False)
            if not blocked_recent.empty:
                st.dataframe(
                    blocked_recent[["Username", "Timestamp", "Risk Decision", "Reasons"]].head(10)
